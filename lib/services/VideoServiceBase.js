...
class VideoServiceBase {
  ...
  getVideoUrl(_videoID, _noCookie) {
    throw new Error("not implemented");
  }

  getFilteredVideoUrl(videoID, noCookie) {
    let filterUrlDelegate = typeof this.env.options.filterUrl === "function"
        ? this.env.options.filterUrl
        : defaultUrlFilter;
    let videoUrl = this.getVideoUrl(videoID, noCookie);
  ...
  getEmbedCode(videoID) {
	...
	let noCookie = this.env.options.nocookie !== undefined && this.env.options.nocookie === true;

    let iframeAttributeList = [];
    iframeAttributeList.push([ "type", "text/html" ]);
    iframeAttributeList.push([ "src", this.getFilteredVideoUrl(videoID, noCookie) ]);
